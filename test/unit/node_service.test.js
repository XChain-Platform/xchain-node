...
describe('NodeService: installNode()', function () {
    ...
    it('installs regtest miner when network is regtest', async function () {
        ...
        const ns = proxyquire('../../src/services/node_service', {
            'child_process': { execFile: stubs.execFile },
            ...
            './docker_service':   { createDockerNetwork: sinon.stub().resolves(), forceRemoveContainerByName: sinon.stub().resolves(true), stopContainerByName: sinon.stub().resolves(true) },
            './database_service': {
                buildDatabaseModule:   sinon.stub().resolves(),
                setDatabaseParameters: sinon.stub().resolves()
            },
    ...
    it('does NOT install regtest miner when network is mainnet', async function () {
        ...
        const ns = proxyquire('../../src/services/node_service', {
            ...
            './docker_service':   { createDockerNetwork: sinon.stub().resolves(), forceRemoveContainerByName: sinon.stub().resolves(true), stopContainerByName: sinon.stub().resolves(true) },
            './database_service': { buildDatabaseModule: sinon.stub().resolves(), setDatabaseParameters: sinon.stub().resolves() },
            './module_service': { cloneGit: cloneGitStub, buildAndUp: buildAndUpStub, assertNoHostPortConflicts: sinon.stub().resolves() },
            './bootstrap_service': { utxoTrackerVolumeFreshness: sinon.stub().resolves('populated'), FRESHNESS_EMPTY: 'empty', ensureBootstrapUtxoTracker: sinon.stub().resolves(), forceBootstrapRequested:     () => false }
        })
    ...
    it('bootstraps utxo tracker when volume is fresh', async function () {
        ...
        const ns = proxyquire('../../src/services/node_service', {
            ...
            './docker_service':   { createDockerNetwork: sinon.stub().resolves(), forceRemoveContainerByName: sinon.stub().resolves(true), stopContainerByName: sinon.stub().resolves(true) },
            './database_service': { buildDatabaseModule: sinon.stub().resolves(), setDatabaseParameters: sinon.stub().resolves() },
            './module_service': { cloneGit: sinon.stub().resolves(true), buildAndUp: sinon.stub().resolves('e'.repeat(64)), assertNoHostPortConflicts: sinon.stub().resolves() },
            './bootstrap_service': {
