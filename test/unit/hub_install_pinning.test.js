...
function load(stubs) {
    return proxyquire('../../src/services/hub_service', {
        ...
        './status_service': { statusChanged: async () => {}, getStatus: async () => {}, getInstalledCoinsAndNetworks: async () => ({}) },
        './docker_service': { addContainerToNetwork: async () => true },
        './module_service': { cloneGit: stubs.cloneGit, buildAndUp: async () => 'id' },
        './database_service': { addUserPasswordToDatabase: async () => {}, getExternalDbConfig: async () => ({}) },
        './db_credential_drift': { readContainerEnv: async () => ({}), assertNoHubDbCredentialDrift: async () => {} },
        './hub_connector.js': class { async ping() { return false } },
        ...
        './release_manifest_service': manifest
    })
}
