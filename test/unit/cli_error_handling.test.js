...
function loadCli(opsOverrides = {}) {
    ...
    const cli = proxyquire(CLI_PATH, {
        ...
        './utils/command_lock': { acquireCommandLock: sinon.stub().returns(function release() {}) },
        './services/telemetry_service': { maybeReportTelemetry: sinon.stub().resolves() },
        ...
        './services/status_service': { getStatus: sinon.stub().resolves(), statusChanged: sinon.stub().resolves() }
    })
    cli.parseCommand()
    return captured
}
...
describe('CLI unhandled-rejection backstop', function () {
    ...
    it('keeps the stack when the reason is a real Error', function () {
        ...
        expect(errorStub.firstCall.args[0]).to.include('cli_error_handling.test.js')
        expect(exitStub.calledWith(1)).to.be.true
    })
})
