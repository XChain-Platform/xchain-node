 ...
 *
 *   text       any literal run of `xchain-node/src/<path>` in any text file:
 *              a relative require, a comment, a shell script, a markdown runbook.
 *   join       a path built segment by segment, the shape
 *              path.join(root, 'xchain-node', 'src', 'foo.js'). When every
 ...
 *              `const R = path.resolve(__dirname, '../../xchain-node')` then
 *              path.join(R, 'src', 'foo.js'), path.join(R, 'src/foo.js') or
 *              `${R}/src/foo.js`. The root may equally be
 *              process.env.XCHAIN_NODE_PATH, a candidate array filtered to its
 ...
 *              this checkout, `nodeFile('src/rollback.js')`. A call over a
 *              literal array, `nodeFile('src/' + twin)`, is emitted once per
 ...
 *              `copy_twin xchain-node "src/$f"`, resolved against the literal
...
// `xchain-node/src/<path>`, however it was spelled: a relative require
// (`../../xchain-node/src/utility.js`), a prose mention, a shell path.
const TEXT_REFERENCE = new RegExp(`${NAME_RE}\\/(src\\/[A-Za-z0-9_@.\\-/]+)`, 'g');

// path.join(..., 'xchain-node', 'src', ...): the tail is captured raw and
 ...
 * may omit the extension (`require('.../config_service')`) and may name a
...
// Environment variables that could hold a checkout of this repo, named the way
// each sibling names its own: <REPO>_PATH and <REPO>_DIR. A path joined onto one
// is a reference like any other, so the sweep resolves it the same way whether
// or not a sibling reads the variable today.
const ENV_ROOT_VARS = ['XCHAIN_NODE_PATH', 'XCHAIN_NODE_DIR'];
...
function rootPrefix(init) {
    ...
    while ((m = re.exec(init)) !== null) {
        ...
        // path.join(base, 'xchain-node', 'src', 'sql').
...
function collectRootProducers(text) {
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        // `const p = nodeFile('src/x.js')` look like another checkout.
 ...
 * checkout, the `nodeFile('src/rollback.js')` idiom. Recognised by a body
...
function helperReferences(text, helpers, lists, strings) {
    ...
    for (const [name, suffix] of helpers) {
        ...
        let m;
        while ((m = re.exec(text)) !== null) {
            // `function nodeFile(rel){` reads as a call of itself. Its
 ...
 * relative path, `copy_twin xchain-node "src/$f"`, where the file comes from
