...
- The release manifest repins `xchain-sync` to v0.19.1 (its SQL-directory boot fix); every other component keeps its v0.19.0 pin and the armed activation heights are unchanged.
...
### Changed
- Patch train carrier: the release manifest moves xchain-indexer to 0.16.1 (the anchor row writer fix that let a from-genesis DOGE testnet sync stop at the first legacy anchor); every other component keeps its v0.16.0 or v0.15.0 tag.
