...
const { XChainService, DB_MODULE_NAME, SEP, tmpDir, BOOTSTRAP_BASE_URL, EXTERNAL_DB } = require('../config')
const { schemaSizeSql, tableExistsSql }               = require('../db/information_schema')
const { tipHeightSql, rowCountSql }                   = require('../db/blocks')
const { db }                                          = require('../state')
const { getDefaultConfig, getModuleDatabaseName, getUtxoTrackerVolumeName } = require('./config_service')
const { stopContainer, startContainer }               = require('./docker_service')
const { getDatabaseContainerId, getDatabaseContainerPresence, ensureDatabasePool, getExternalDbConfig, executeNativeMariaDbCommand } = require('./database_service')
const { assertSafeArchiveMemberNames, redactSecrets } = require('../utils/helpers')
const { buildBootstrapMeta, writeBootstrapMeta } = require('./bootstrap_archive_meta')
...
async function assessNodeTipBeforeRestore(coin, network, module, archivePath) {
    const { assessNodeTipForRestore } = require('./bootstrap_node_tip_guard')
