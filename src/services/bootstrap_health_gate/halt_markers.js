...
function assertWatermark(value, what) {
...
// return its raw `-BN` output: over the native client in external-DB mode,
// otherwise through `docker exec` into the database container.
function sqlRunner(coin, network, deps) {
    const {
        runner,
...
function countReader(run) {
    return async (sql, what) => {
...
async function probeDatabase(run, readCount, name, since) {
    const { found, hasSyncHalt } = await readLiveMarkers(run, readCount, name)

    // Nothing recorded from an earlier reading: this call is the reading.
    if (!since) return found

    found.raisedInWindow = await readRaisedInWindow(readCount, name, since, found, hasSyncHalt)
    return found
}

// Read which marker tables exist in `name`, how many live halt rows each holds,
// and the id watermark of each, for probeDatabase.
async function readLiveMarkers(run, readCount, name) {
    ...
    return { found, hasSyncHalt }
}

// Count the halt rows raised in `name` since the watermarks in `since`, for
// probeDatabase. `found` carries the watermarks this reading took.
async function readRaisedInWindow(readCount, name, since, found, hasSyncHalt) {
